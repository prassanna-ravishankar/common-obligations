...
Checkpoint `86f1940` preserves the approved fixed-region comparison edition. Subsequent user request: slide-like question transitions across all scenarios, delayed rust emphasis in full-screen statements, a background handoff into the next topic, and modular code. Current implementation uses shared QuestionSequence/Statement components, one scene per scenario's question sequence, and separate scripts/reading modules with shared timing.js; reading-sequences.css replaces superseded comparison rules. Surveillance and incident dissolve between three questions inside one pinned scene; all six use the same question-to-perspective grammar. Statement emphasis reveals after a 700ms dwell, with complete static motion-off alternatives. Model/agent diagram remains unboxed. No new publication approval.

After checkpoint `0dbd392`, the user requested spatial continuity in comparisons: each perspective occupies its final half/third from the start, attention shifts between those regions, then all resolve in place. The current Comparison component renders each argument once inside a shared stage, which subsequently unpins; no repeated comparison section, instructional heading or counter. A keyboard-focus-only bypass and direct comparison hashes resolve the shared composition. All ten browser checks pass, including stable region geometry, reverse scrolling, complete no-JS content and static mobile/short/motion-off layouts. Chapter opening artwork is unchanged.
